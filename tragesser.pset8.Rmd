...
This Z score allows us to reject the null hypothesis that the average PC income is the same between the Obama and McCain states.
...
The null hypothesis, or $H_0$, is that the average log-household income for Pakistani households is log-38,000 Pakistani Rupees/year. The alternative Hypothesis $H_1$, is that the average log-household income is different from log-38,000 in either direction. We are calculating the one sample t-test.

### Part B

Hypothesis testing of means relies on the central limit theorem to calculate z-scores, which would not be achieved with a sample size of 12. 
