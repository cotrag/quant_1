...
```{r}
draws_normal <- replicate(10000, rnorm(30))
draws_df <- as.data.frame(draws_normal)

draws_df <- draws_df %>% 
  mutate(across(everything(), ~sum((. - mean(.))^2)/29, .names = "sample_var_{.col}")) %>% 
  mutate(across(V1:V10000, ~(sum((. - mean(.))^2)/29) * 29, .names = "other_calc_{.col}"))
...
```{r}
draws_uniform <- replicate(10000, runif(30, min = -3, max = 3))
draws_df <- as.data.frame(draws_uniform)

draws_df <- draws_df %>% 
  mutate(across(everything(), ~sum((. - mean(.))^2)/29, .names = "sample_var_{.col}")) %>% 
  mutate(across(V1:V10000, ~((sum((. - mean(.))^2)/29) * 29)/((1/12)*((3 + 3)^2)), .names = "other_calc_{.col}"))
...
draws_df <- draws_df %>% 
  mutate(across(everything(), ~(mean(.)-1)/(1/(30^(1/2))), .names = "t_{.col}"))
...
draws_df <- draws_df %>% 
  mutate(across(everything(), ~ (mean(.)-((-3+3)/2))/((sum((. - mean(.))^2)/29)/(30^(1/2))), .names = "t_{.col}"))
...
regression_manual2 <- function(x, y, N) {
  ...
  # for (i in c(input_x,input_y)) {
  #   input_i <- deparse(substitute(i))
  #   print(input_i)
  #   
  #   if ("i" == "x") {
  #     z = "i"
  ...
  #   
  #   print(col_name)
  #   mean <- paste("mean_", i, sep = "")
  #   dev <- paste("dev_", i, sep = "")
  #   devsq <- paste("devsq_", i, sep = "")
  #   sd <- paste("sd_", i, sep = "")
  # 
  #   # Create a new object with the generated name
  #   assign(mean, mean(i))
  #   print(mean)
  #   assign(dev, i - mean(i)) 
  #   assign(devsq, (i - mean(i))^2)
  #   assign(sd, (sum(dev_isq)/(N-1))^(1/2))
  # }
  # dev_mult_sum = sum(dev_x*dev_y)
  # pearson = dev_mult_sum/((sum(devsq_x) * sum(devsq_y))^(1/2))
  # beta_1 = pearson * (sd_y / sd_x)
  # beta_0 = mean_y - (beta_1 * mean_x)
  # print(paste("The coefficient for X is", beta_1, "and The intercept is", beta_0))
}
